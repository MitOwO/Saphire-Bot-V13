# 👩‍💻 Centralzinha de Ajuda Técnica e Comandos
Esta página no modo escuro é 500% mais bonita.

### Links

[Servidor Principal/Suporte](https://discord.gg/3g8Sa4dUc9)

[Convite para me adicionar no seu servidor](https://discord.com/api/oauth2/authorize?client_id=837147659898191902&permissions=8&scope=bot)

# ℹ️ Observações importantes

Prefixo padrão: ( - )

- Simbolos nos comandos

• `-ban [@fulano]` Itens dentro de [ ] são **argumentos obrigatórios** para que os comandos funcionem

• `-avatar <@membro>` Itens dentro de <> são **argumentos opcionais**, o comandos irão funcionar com ou sem eles

• `-pay <@membro/id/menção>` Itens com </> são **argumentos de escolha**, você pode escolher quais argumentos usar

• ~`-delete all`~ Comandos riscados indenficam **reforma/manutenção/não lançado**. Portanto, estarão fora do ar.

# 👤 Saphire
![](https://github.com/rodycouto/Saphire-Bot-V13/blob/main/Routes/Images/SaphireIcon.png)
##### Art Profile By: [Pepy#1000](https://discord.gg/RqhzDRypsS) *from Saphire's Team*

### Olá, meu nome é Saphire!
Eu fui criada originalmente para ser uma bot de administração em um servidor pequeno que não existe mais.

Estou em vários servidores brincando e me divertindo com a melhor galera do mundo.

Sabia que sou portadora de centenas de comandos? Tenho desde os fofos hug/kiss até sistemas super avançados que protegem o seu servidor 24 horas por dia.

Hoje, além de uma simples bot de simples de diversão, me tornei uma bot capaz entregar segurança, diversão, entretenimento, automação do seu servidor, economia, administração pesada e personalizada e um sistema global de experiência, afk, e eu cansei de escrever... Olha aí pra baixa, tá bom?

Explore tudo o que eu tenho e seja uma pessoa melhor para não entrar na minha blacklist 🔪

## 📃 Informações super interessantes
- **Nome:** Saphire
- **Idade**: 17 anos
- **Aniversário:** 29/04/2021 *(Data de criação)*
- **Hobby:** Interagir com a galera do Discord
- **Matérias Preferidas:** Ciência e Técnologia
- **Objetivo:** Assistir todas as séries da [Netflix](https://www.netflix.com/br/) e ter cada vez mais usuários pra brincar comigo
- **Gostos:** Músicas, animes/filmes/séries, laços azuis e sorvete de montão
- **Minha Casa:** [Meu lindo servidor](https://discord.gg/3g8Sa4dUc9)
- **Host:** [DisCloud](https://discloudbot.com/) - Plano Atual: Gold
- **Tempo online:** Aproximadamente 29hrs por dia
- **Linguagem:** JavaScript, Discord.js, Node.js

## 🛡️ Saphire's Team
- A Saphire's Team é privada e cada um tem seu papel. Esta equipe trabalha de forma independente. Cada um tem seu senso de responsabilidade e não é de sua obrigação cumprir seu papel a risca.
Nenhum fins lucrativo é visão ou meta. Todas as doações da Saphire vão para a Saphire, envolvendo Host, equipamentos e gastos envolvendo a parte WEB/Técnica

## Membros

#### Rody#1000 - ***Official Developer***
- Criador e desenvolvedor da Saphire
    - Responsável por tudo referente ao código fonte da Saphire. Como resolução de bugs e implementação/manutenção de novos comandos

#### Raffah#1751 - *General Administrator*
- Administradora Geral
    - Responsável pela logística técnica da Saphire e decisões que aplicam o meio global

#### Mikah#5092 - *Manager & Coordinator*
- Gerência e Coordenação Técnica
    - Responsável pelo bem-estar da Saphire, Servidores, Equipes e Doações
        - Gerênciamento de assuntos técnicos e suporte direto a resoluções de problemas
      - Edições na parte frontal e designer de mensagens a resposta de comandos

#### Gowther#9233 - *Content Anime Manager*
- Gerência de Conteúdo de cunho Anime
    - Responsável por todo conteúdo presente na Saphire que envolva Anime

#### MakolPedro#4369 - *Analyst and Command Reviewer*
- Analista técnico de comandos
    - Responsável por buscar possíveis erros na parte frontal da Saphire e de forma técnica e pesada

#### Pepy#1000 - *Official Designer, Emojis Productor & Events Creator*
- Designer Oficial
    - Responsável por todo e qualquer desenho original da Saphire, produção e alterações de emojis e criação de eventos exclusivos

# 📢 Comandos de Reportes e Sugestões
- No abuso destes comando, a Saphire's Team te colocará na lista de membros bloqueados e você perderá o acesso a estes comandos permanentemente.

##### `-gif <tema> <link>` - Envie gifs pra serem adicionados ao package da Saphire
O Link **deve** ser enviado junto ao **tema** no qual você deseja que o Gif seja implementado.
Comando exemplo: `-gif Mirai https://imgur.com/F1nJKHZ`

##### `-sugest` Mande suas sugestões para o [meu servidor](https://discord.gg/3g8Sa4dUc9)
Formule bem suas sugestões para não serem recusada/mal compreendida.
Suas sugestões **devem** ser redirecionadas a Saphire! Como algum novo comando ou alguma edição.

`-bug [Descrição do ocorrido]` Envie bugs/erros que você notou direto pro Rody#1000, o criador da Saphire

`-donate` Tem um real sobrando? Me ajuda poxa

`-sendcantada` Cantadas pro comando `-cantada`


# 🌐 Saphire Global System

Com o Sistema Global da Saphire, torna possível o compartilhamento de dados entre servidores. Sendo assim, você vai ter as mesmas coisas em todos os servidores em que a Saphire estiver presente.
- Isso se aplica aos Sistemas de [Economia](https://github.com/rodycouto/Saphire-Commands/blob/main/README.md#-o-lindo-sistema-de-economia-global), [Experiência](https://github.com/rodycouto/Saphire-Commands/blob/main/README.md#%EF%B8%8F-level-system---uhuul-level-up) e [AFK](https://github.com/rodycouto/Saphire-Commands/blob/main/README.md#o-bel%C3%ADssimo-afk-global-command)
- Sistemas Locais não existem na Saphire. Então, se você estiver procurando como ativar o *Ranking Local* por exemplo, você não irá achar.
- Não peça por esse sistema. A Saphire's Team **não** vai implentar o sistema local por ser inviável e sem utilidade (até o momento) na Saphire

## ℹ️ Acesso rápido as categorias

Devido a quantidade de comandos, as categorias foram criadas para facilitar o acesso e busca a eles.

Caso você esteja procurando algum comando em especifico, primeiro veja a qual categoria ele pertence. Os nomes das categorias são bem intuitivos então isso não será um problema.

# 🌟 Listinha dos meus lindos comandos

## O Belíssimo AFK Global Command
- Com esse sistema, você pode ficar tranquilo! Quando alguém te chamar, eu vou avisar que você está offline e mostrarei a mensagem que você deixar, tipo um recado.

`-help afk` Informações úteis, uma boa explicação e exemplos fáceis do comando.

`-afk` Ative o AFK no seu servidor sem nenhuma mensagem.

`-afk < sua mensagem >` Ative o AFK com uma mensagem personalizada.

- Ative o AFK em todos os servidores. SIIM, em todos!

`-afk [global/all] < sua mensagem >`
- ##### O AFK System será desativado no momento em que você mandar alguma mensagem.

# 💸 O Lindo Sistema de Economia Global
• Ganhar e perder dinheiro nunca foi tão divertido, agora você pode fazer isso brincando.

Moeda: ⭐RPoints *(Saphire Points)*

- ##### Administração
- `-setpescachannel #canal` Canal para pesca
- `-setminechannel #canal` Canal para mineração
- `-setbuscachannel #canal` Canal para busca
- ~`-seteconomychannel #canal`~ Canal de economia *(Habilita sistema de economia)*

Veja o seu saldo *(adicione @user para ver o saldo de um amigo ***-saldo @user***)*
- Traduções: `-bal` `-balance` `-atm` `-money` `-conta` `-rp` `-saldo`

### ❓ O que tem no Sistema de Economia da Saphire?

- A Saphire não se responsabiliza por dinheiro perdido ou mal usado.

- Lojinha chique: `-shop`
- Inventário Tecnológico: `-slot`
- Pagamentos: `-pay [@user] [valor]`
- Sistema de vendas: `-sell <@user> [item] [quantidade]`
- Deposite: `-dep [valor]/[all]`
- Saque: `-sacar [valor]/[all]`
- Roube: `-rob [@user]`
- Jogo: `-blackjack [valor]`
- Jogue dinheiro no chat: `-lance [valor]`
- Cobre: `-cobrar [@user] [valor]`
- Mendigue: `-esmola`
- Suba no ranking: `-rank money`
- Cace tesouros: `-pesca` `-minerar` `-buscar`
- Libere funções: `-settitulo` `-assaltar [@user]` `-pesca` `-minerar` `-buscar`
- Ajude a Princesa Kaya: `-floresta` *Mais comandos neste comando*
- Duele: `-duelar [@user] [Valor]`
- Jogue na loteria: `-loteria`
- Aprimore seu perfil: `-perfil`
- Envie cartas: `-carta [@user] [Sua mensagem]` ~~*(+5 Reputação)*
- Customize sua cor: `-buy [cor]`

### ❓ Como ganhar dinheiro?

- Ganhe dinheiro: `-work` `-daily` `-vender`
- Aposte na sorte: `-slut` `-cu` `-blackjack` `-crime` `-roleta` `-lotery/-loteria` `-duelar [@user] [valor]`
- Venda itens que você coleta com a `-pesca` `-cavar` `-buscar`, utilize `-sell [item] [quantidade]`. Os baús escondidos são uma boa fonte de renda.
- Ajudar a Princesa Kaya e vender Diamantes são atos preciosos. `-floresta`

- Você pode ser preso fazendo coisas ruins, como roubar ou entrar no mundo do crime, então tome cuidado. Mas se você for preso, você pode tentar `-fugir`, se der azar, a pena aumenta e você não pode mais usar o sistema de economia até o tempo zerar.

### Ajudinha rápida sobre a economia da Saphire

`-help economia` Todos os comandos que aqui estão mais informações que também aqui estão.

`-bjhelp` Não sabe jogar blackjack? Veja [como jogar](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/comojogarbj.md)

`-dogname [NomeDoDoguinho]` Mude o nome do seu cachorrinho após a ajudar a Princesa Kaya `-floresta`

- Traduções suportadas: `-depositar` `-withdraw` `-lotery` `-store` `-buy` `-pagar` `-doar` `-roubar` `-trabalhar` `-prostituir` `-slot` `-mendigar` `-cavar` `-buy` `-comprar` `-inve` `-inventario` `-roubar` `-rol` `-sell` `-loja` `-vender`

# 🌎 Chat Global

Fale com a galera dos outros servidores através do Chat Global.

- `-chat [A sua mensagem global]` Envie sua mensagem para todos os servidores com o sistema ativado.
- `-channel create text/voice canal` Crie um canal se você quiser.
- `-setglobalchat #canal` Escolhe um canal para ser o Global Chat
- `-regraschat` As regras do Global Chat
- `-globalchat` Ajudinha de como ativar o sistema.

##### A Saphire não se responsabiliza por quaisquer mensagem inapropriada! *(Existem moderadores para mutar/banir pessoas do chat global.)*

# ⭐ VIP / Donate - Só os melhores tem

*Antes de tudo, fique ciente de que o VIP System não dá previlégios ou vantagens a ninguém. O VIP System é uma forma de agradecimento e libera funções que não dão vantagens, apenas é legal tê-las.*

Use `-vip` que te mando todas as informações

- ❓ Como obter o VIP?

Simples! Você pode fazer uma doação de [R$1,00](https://mpago.la/2jYiNDg) no Mercado Pago ou fazer um PIX para o meu PicPay, basta digitar `-pix`.

Você pode ter mais informações de como doar enviando `-help` ou `-donate`.

- ❓ O que eu ganho com o VIP?

##### Comandos Vips

- `-shop vip` Um loja exclusiva para os vips
- `-setcolor [cor]` Escolha a cor das suas embeds/mensagens
- `-slot vip` Um slot só pros vips
- `-buy estrela5` Compre a 5º Estrela
- `-esmola` Mendigue dinheiro para os outros
- ... Mais comandos estão sendo escritos ...

- ❓ Como comprovar o pagamento?

Simples! Entre no [meu servidor](https://discord.gg/3g8Sa4dUc9) e fale com o **Rody#1000**, enviando um **print do comprovante** e pronto, você tem seu VIP.

Se quiser mais bônus no servidor principal, fale com **MakolPedro#4369**.

# ⬆️ Level System - Uhuul, level up!
• É isso meeesmo! Level, nível, xp, ranking, tabelinha de xp, tudo isso aí. Lembrando, é interservidor, se você passar de nível no seu servidor, você também vai passar de nível no servidor do seu amigo. 

`-level <@user>` Veja seu level ou o de alguém

`-daily` Pegue sua dose diaria de dinheiro e xp

`-rank xp` Ranking Interservidor de XP Saphire

~`-levelrole [level]`~ Dá um cargo para quem alcançar um level especifico

- Por ser global, os administradores não pode dar ou tirar xp, i'm sorry.

### Family System
- Adicione pessoas ao seu perfil

`-marry [@user]` Se case com alguém

`-divorce [@user]` Se divorcie

`-family` `-nofamily` Informações e comandos que aqui estão

`-family1` `-family2` `-family3 [@user]` Adicione até 3 pessoas a sua familia

`-nofamily1` `-nofamily2` `-nofamily3 [@user]` Tire alguém da sua familia

### Perfil

`-rep [@user]` Dê reputação

`-setstatus [Seu Status]` Defina seu status no seu perfil Exemplo:
*-setstatus Eu sou a lua que brilha no céu*

`-profile` `-perfil < @user >` Veja seu perfil ou o de alguém

`-settitulo [Seu título]` Defina seu título *compre na `-loja`*

`-setsigno` Defina seu signo

`-setniver [DD / MM / AAAA]` Defina seu aniversário

# 💿 Music System

• O Music System foi desativado por que a API do *YouTube/Spotify/SoundCloud* não colaboram comigo.

# 🛠️ Comandos Administrativos
• Isso aqui é um paraiso para os Mods e Adms

#### 🔸 Areazinha da Personalização

`-setxpchannel [#canal]` Envio de mensagens Level Up. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setxpchannel.md)

`-setreportchannel [#canal]` Receba report dos membros. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setreportchannel.md)

`-setleave [#canal]` Canal de saída. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setleave.md)

`-setleavemsg [Sua mensagem de saída]` Escolha uma mensagem legal pra saida. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setleave.md)

`-setwelcome [#canal]` Canal de boas vindas. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setwelcome.md)

`-setwelcome off` `-setleave off` Desative o sistema de boas vindas/saídas

`-setwelcomemsg [Sua mensagem de boas vindas]` Escolha uma mensagem legal de boas vindas. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setwelcome.md)

`-setlogchannel [#canal]` Canal de Relatórios. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setlogchannel.md) *(Libera funções como ban/mute etc...)*

`-setpescachannel #canal` Canal de pesca

`-setminechannel #canal` Canal de mineração

`-setbuscachannel #canal` Canal de busca

`-setautorole [@cargo]` Selecione um cargo para novos membros do servidor recebe-lo automaticamente

`-setautorole off` Desative o autorole no seu servidor

`-autorole` Veja o estado de autorole do seu servidor

`-role create/delete [Nome do Cargo]` Crie e delete roles

`-role edit name/color [@role] [Novo name/color]` Edite nomes e cor dos cargos

`-role membros [@role]` Veja quantos membros tem em algúm cargo

`-role id [@role]` ID do cargos

#### 🔸 Moderação

`-move [@user]` Move um membro pra sua call

`-addrole [@user] [@cargo]` Dê cargos

`-removerole [@user] [@cargo]` Tire Cargos

`-setnick [@user] [Novo Nome]` Mude o nickname de alguém

`-kick [@user] <razão>` Chute alguém do servidor

`-ban [@user] <razão>` Banir membros

`-unban [ID]` Desban algúm membro *(requer o ID do usuário)*

`-muteinfo` Informações do Mute System

`-mute` Ativa as configurações do comando mute *(Mova a role Muted para um cargo alto)*

`-mute [@user] [10s/m/h] <razão>` Mute alguém

`-unmute [@user] <razão>` Desmuta alguém

`-warn [@user]` Dê Warns *razão em breve*

`-warns [@user]` Confira Warns

`-rwarns` `-resetwarns [@user]` Reset warns de alguém

`-kickvoice [@user]` Tire alguém da call

### 🔸 Chat Commands

`-clonechannel` Cria um canal com o mesmo nome

`-channel create text/voice [#NomeDoCanal]` Crie canais de texto/voz

`-channel topic [NovoTópico]` Edite o tópico do canal

`-channel name [#channel] [novo nome]` Mude o nomes do canais

`-lockdown` Trave o servidor em caso de emergência

`-lock` Trave um canal especifico

`-unlock` Destrave um canal especifico

`-lockcommands <#canal>` Bloqueie meus comandos em canais especificos

`-unlockcommands <#canal>` Libere meus comandos em canais que foram bloqueados

`-clear [all]` Limpe todo o chat

`-clear [1~99]` Apague de 1~99 mensagens

`-clear <@user> [1~99]` Apague 1~99 mensagens de alguém especifico

`-clear <bot>` Apague mensagens de bots

`-clear <images> [1~99]` Apague somente imagens/videos/gifs

- ##### Mensagens acima de 14 dias não serão apagadas por limitações do Discord.

`-slowmode [Tempo em segundos]` Ative o modo lento

### 🔸 Saphire Comandos Exclusivos

`-setprefix [NovoPrefixo]` Muda meu prefixo

`-prefix` Informações sobre o prefixo

`-resetprefix` Reseta meu prefixo para ( - )

`-dm` `-pv [@user] [Mensagem]` Comando Travado

`-ideiasaphire [Sua ideia]` Envia sua ideia diretamento pro servidor central. 100% de certeza de que você será ouvido.

`-gif [linkdogif.com]` Envie gif e aumente os packs da Saphire

### 🔸 Interação com os server

`-criarcomando [NomeDoComando] [O que eu vou responder]` Cria um comando só pro servidor

`-deletecomando [NomeDoComando]` Deleta um comando criado

~`-listadecomandos`~ Em breve

`-send [#canal] [Sua mensagem]` Envie mensagens para outros canais

`-say [Sua mensagem]` Mande mensagens atráves de mim

`-sorteio [20s/m/h/d] [#CanalDoSorteio] [Prêmio]` Faça sorteios

# 🎮 Link de Games

• Acesse os principais jogos rapidamente

`-amongus`

`-brawlhalla`

`-brawlstars`

`-clashroyale` `-clash`

`-counterstrike` `-cs`

`-ddtank`

`-freefire` `-ff`

`-gartic`

`-genshinimpact` `-genshin`

`-gta`

`-habbo`

`-leagueoflegends` `-lol`

`-summoners` `-sumwar`

`-mario`

`-mobilelegends` `-mbl`

`-minecraft`

`-paladins`

`-rocketleague`

`-transformice` `-tfm`

`-uno`

`-valorant`

# 📺 Animes

- Esses comandos estão sendo desenvolvidos ainda, não se preocupe

~`-anime [Seu anime]`~ Pesquisa de animes

`-ind` Indicação de Animes (+2000 Animes)

`-sao` Fotos/gifs de Sword Art Online

`-senpai` Meu lindo e perfeito senpai

`-quiznaruto` Quiz de Naruto pra quem é viciado

`-quizonepiece/op` Tem de One Piece também

`-quizkimetsu` Kimetsu no Yaiba também está na lista.

#### Pack de Gifs/Fotos

`-asuna` | `-chika` | `-nezuko` | `-zerotwo/-zt`

`-gojo` | `-zenitsu` | `-naruto` | `-itachi`

`-kimetsu` Pack de Gifs de Kimetsu No Yaiba

- Em breve: `-kirito` e mais 36 packs *(Tô com preguiça de escrever)*
# 💡 Comandos Gerais

• Comandos úteis e que podem ajudar muito

`-covid < br >` Informações mundiais sobre a COVID-19

### 🔸 Servidor

`-invite` `-inv` Me coloque no seu servidor

`-ideia [Sua ideia]` Mande ideias pro seu Servidor para ser votada

`-report <@user> [O motivo do seu report]` coisas pra Staff do Servidor

`-serverinfo` Informações do Servidor

`-dono` `-owner` Veja quem é o dono do servidor

`-invites` Veja quantas pessoas cada membro convidou pro servidor *(Criador do link que ganha o invite)*

### 🔸 Comandinhos da Saphire

`-ping` Pong!

`-uptime` Tempo que eu estou acordada

`-botinfo` Informações sobre mim

`-sup` `-support` `-suporte` Minha central de Suporte

### 🔸 Usuário

`-avatar <@user>` Veja sua foto de perfil ou a de alguém

`-id <@user>` Veja o ID

`-spotify` `-spot <@user>` Veja a música que alguém está escutando

`-user <@user>` Veja o nome de usúario de alguém

`-userinfo <@user>` Veja informações detalhadas de alguém

### 🔸 Fun

`-Jokenpo` `-j < pedra > < papel > < tesoura >` Jooo keeen po!

`-coinflip < cara > < coroa >` Cara ou coroa?
 
~`-ascii [Sua mensagem]`~ Muito doido

`-dado` `-roll [1...4]` Role até 4 dados

`-frase` Filosofia pura

~`-changemymind [Sua frase]`~ Changemymind Meme (Not Working)

`-history` Histórias criadas pelo pessoal do Discord

### 🔸 Util

`-cat` Gatinhos são úteis, tá bom? ;-;

`-clima [SuaCidade]` Veja o clima da sua cidade

`-cal [10 + 10]` Calculadora poxa

`-translate` `-t [pt][en][fr] [A frase que deseja traduzir]` Traduza com o Google Tradutor *Exemplo: -t pt What are you mean with that?*

`-invisible` `-invisivel` Não há nada aqui

`-piada` kkkkkk q u a l i d a d e

`-imc [peso] [altura]` (Peso e Altura sem ponto e virgula) Veja seu imc *Exemplo: -imc 75 181*

### 🔸 Random

`-advice` This command is completelly in english

`-conselho` Esse comando é completamente em português

`-invisivel` `-invisible` Invisivel

`-nota [@user]` Dê nota a alguém

`-boom` Só faz boom

`-cantada` Cantadas muito boas

`-explosion` Megumiiin

`-feet` Pézin

`-servidores` Servidores que usam a Saphire

`-livia` Gay

`-loli` Lolicon

`-stonks` `-notstonks` Stonks esse comando

`-puppy` Fofiiiinho

# ❤️ Comandos de Reações
• Demonstre reações por gifs no servidor

`-clap` Aplaudir é legal

`-cry` Chorar as vezes faz bem

`-dance` Dance Dance

`-ery` Puff.. kkkk

`-facepalm` Decepcionado(a)

`-fury` Mostre sua furia!

`-glare` Olhar Pistola

`-happy` Feliiiz

`-kkk` kkkkkk

`-pout` Fazendo Bico

`-run` Coooorre

`-sad` Triste

`-shy` Vergonha

`-sleep` Que soninhoo

`-smile` Está feliz? Sorria!

`-surprise` Mas o que é isso?

`-thin` Oque eu faço... Pensando...

`-tired` que cansaço!

`-trig` TRIGGEEEEER

- Todos tem traduções para português.

# 🤝 Comandos de Interação
• Interaja com as pessoas atráves de gifs sensacionais

`-votação` `-votar` `-vote [O que deseja ser votado]` Abra um votação

`-baka [@user]` Chame de Baka!

`-morder` `-bite [@user]` Mordidinha

`-dedo [@user]` Educação

`-saudar` `-comprimentrar` `-greet [@user]` Comprimento

`-fight [@user]` Briga, briga, briga!

`-lutar [@user]` Lutinha saudavel

`-abraço` `-abraçar` `-hug [@user]` Abraçar é tão boom

`-matar` `-kill [@user]` Mate!

`-beijo` `-beijar` `-kiss [@user]` Hmmm, beijo é?

`implorar` `-onegai [@user]` Por favoor

`-carinho` `-pat [@user]` Cariiiinho

`-rob [@user]` Roubar é tão feio...

`-tapa` `-slap [@user]` Tapããão

`-slaap [@user]` Tapão em foto

`-love [@user]` Mande seu amor secretamente

`-ery [@user]` Deboche

`-lamber` `-lick [@user]` Lambidinha

`-chupar` `-suck [@user]` Chupada

`-tocaai` `-highfive [@user]` Toca Aqui

`-cutucar` `-poke [@user]` Cutucadinha

`-atirar` `-shoot [@user]` Atire para matar

`-shrug [@user]` Fazer oque ne

`-olhar` `-stare [@user]` Olhar fixo

`-pisar` `-stomp [@user]` Pise em alguem

`-affect [@user]` Isso não afeta o bebê

`-explodir [@user]` Exploda alguém

- Todos comandos de interação tem traduções, caso queira adicionar mais alguma tradução, [clique aqui](https://forms.gle/vtJ5qBqFDd9rL5JU8)

# 📝 Quiz - BETA
• O Saphire Quiz é um jogo simples e divertido de brincar. Tendo 2 modos, é possível brincar bastante. Jogar com os amigos é ainda mais divertido!!!

`-quiz` Você terá apenas 15 segundos para responder

`-quiznaruto` Você conhece Naruto? Tem 15 segundos também

`-quizonepiece/op` Tem 15s pro One Piece também

`-quizkimetsu` Tem 15s pro Kimetsu também

### Em breve
`-quizanimes` `-quizcinema` `-quizhistoria` `-quizch` -> corpo humano

#### Informações Adicionais

Quiz: 27 Perguntas

Quiz Naruto: 17 Perguntas

Quiz One Piece: 15 Perguntas (24 perguntas em breve)

Quiz Kimetsu no Yaiba: 15 perguntas

# 📁 Saphire Assistence
• Pessoas que ajudaram na construção da Saphire

[Servidor Central](https://discord.gg/3g8Sa4dUc9) | [Host Atual](https://discloudbot.com/)

### Moderação do Servidor Saphire
MakolPedro#4369 | ツ𝑫𝒐𝒎𝒂#6556 | ! Kokushibou#2447 | Raffah#1751

### Créditos a todos que ajudaram

Rody#1000 | `Construção do Código e Pesquisa`

Gowther#9233 | `Start Cooper` *~Developer acess & Gif Collector*

Spray#7194 | `-lock | -unlock` *~ideia*

Heryson#6565 | `AllowedMentions` *~conselho & ajuda*

𝑅𝑈 𝑀𝑖𝑛𝑒?#2790 | `Ideias nos Comandos de Administração` *~ideia*

LucasLuquisquiss#4643 | `-frase` *~ideia*

João da Cilada#7041 | `-love` *~ideia*

MakolPedro#4369 | `-help games` `-quizonepiece` `Atualizador do Help` *~ideias, developer, Perguntas/Respostas Quiz*

Ceci#7184 | `-doar itens` *~ideia*

Giacometti#4849 | `-feet` *~ideia*

! Kokushibou#2447 | ツ𝑫𝒐𝒎𝒂#6556 | `-quizkimetsu` *~ideia + Perguntas/Respostas Quiz*

Dog Comunista#2185 | `-lockcommands` *~ideia*

Coiny#9056 | `Painel Interativo` *~ideia & support*

CALORIES#2822 | `Music System Helper` ~ USA *~ideia, support, developer*

### Caça Bugs

Casado✋🏻😎#9113: `5 Bugs`

Heryson#6565: `2 Bugs`

Pitticco#1827: `1 Bug`

ツ𝑫𝒐𝒎𝒂#6556: `29 Bugs`

Bαყʑι ッ#5485: `4 Bugs`

MakolPedro#4369: `49 Bugs`

! Kokushibou#2447 `15 Bugs`

### Gifs / Emojis

[@milkmochabear](https://www.instagram.com/milkmochabear)

Foto de perfil: [Sayu](https://br.pinterest.com/pin/849843392186157763/)

ツ𝑫𝒐𝒎𝒂#6556 `Emojis servidor`

Gowther#9233 `Gifs Saphire`

dspofu#8041 `Emojis servidor`

Hype#0004 `Emojis Saphire`

! Kokushibou#2447 `Emojis servidor`

¸,ø¤{•ลℓเ¢э-µ¢ђเђล}ø¤º°#6146 `Emojis servidor`

### Designers

Yma#5175, Rody#1000, Gowther#9233, Sayu# 👩‍💻 Centralzinha de Ajuda Técnica e Comandos
Esta página no modo escuro é 500% mais bonita.

### Links

[Servidor Principal/Suporte](https://discord.gg/3g8Sa4dUc9)

[Convite para me adicionar no seu servidor](https://discord.com/api/oauth2/authorize?client_id=837147659898191902&permissions=8&scope=bot)

# ℹ️ Observações importantes

Prefixo padrão: ( - )

- Simbolos nos comandos

• `-ban [@fulano]` Itens dentro de [ ] são **argumentos obrigatórios** para que os comandos funcionem

• `-avatar <@membro>` Itens dentro de <> são **argumentos opcionais**, o comandos irão funcionar com ou sem eles

• `-pay <@membro/id/menção>` Itens com </> são **argumentos de escolha**, você pode escolher quais argumentos usar

• ~`-delete all`~ Comandos riscados indenficam **reforma/manutenção/não lançado**. Portanto, estarão fora do ar.

# 👤 Saphire
![](https://github.com/rodycouto/Saphire-Bot-V13/blob/main/Routes/Images/SaphireIcon.png)
##### Art Profile By: [Pepy#1000](https://discord.gg/RqhzDRypsS) *from Saphire's Team*

### Olá, meu nome é Saphire!
Eu fui criada originalmente para ser uma bot de administração em um servidor pequeno que não existe mais.

Estou em vários servidores brincando e me divertindo com a melhor galera do mundo.

Sabia que sou portadora de centenas de comandos? Tenho desde os fofos hug/kiss até sistemas super avançados que protegem o seu servidor 24 horas por dia.

Hoje, além de uma simples bot de simples de diversão, me tornei uma bot capaz entregar segurança, diversão, entretenimento, automação do seu servidor, economia, administração pesada e personalizada e um sistema global de experiência, afk, e eu cansei de escrever... Olha aí pra baixa, tá bom?

Explore tudo o que eu tenho e seja uma pessoa melhor para não entrar na minha blacklist 🔪

## 📃 Informações super interessantes
- **Nome:** Saphire
- **Idade**: 17 anos
- **Aniversário:** 29/04/2021 *(Data de criação)*
- **Hobby:** Interagir com a galera do Discord
- **Matérias Preferidas:** Ciência e Técnologia
- **Objetivo:** Assistir todas as séries da [Netflix](https://www.netflix.com/br/) e ter cada vez mais usuários pra brincar comigo
- **Gostos:** Músicas, animes/filmes/séries, laços azuis e sorvete de montão
- **Minha Casa:** [Meu lindo servidor](https://discord.gg/3g8Sa4dUc9)
- **Host:** [DisCloud](https://discloudbot.com/) - Plano Atual: Gold
- **Tempo online:** Aproximadamente 29hrs por dia
- **Linguagem:** JavaScript, Discord.js, Node.js

## 🛡️ Saphire's Team
- A Saphire's Team é privada e cada um tem seu papel. Esta equipe trabalha de forma independente. Cada um tem seu senso de responsabilidade e não é de sua obrigação cumprir seu papel a risca.
Nenhum fins lucrativo é visão ou meta. Todas as doações da Saphire vão para a Saphire, envolvendo Host, equipamentos e gastos envolvendo a parte WEB/Técnica

## Membros

#### Rody#1000 - ***Official Developer***
- Criador e desenvolvedor da Saphire
    - Responsável por tudo referente ao código fonte da Saphire. Como resolução de bugs e implementação/manutenção de novos comandos

#### Raffah#1751 - *General Administrator*
- Administradora Geral
    - Responsável pela logística técnica da Saphire e decisões que aplicam o meio global

#### Mikah#5092 - *Manager & Coordinator*
- Gerência e Coordenação Técnica
    - Responsável pelo bem-estar da Saphire, Servidores, Equipes e Doações
        - Gerênciamento de assuntos técnicos e suporte direto a resoluções de problemas
      - Edições na parte frontal e designer de mensagens a resposta de comandos

#### Gowther#9233 - *Content Anime Manager*
- Gerência de Conteúdo de cunho Anime
    - Responsável por todo conteúdo presente na Saphire que envolva Anime

#### MakolPedro#4369 - *Analyst and Command Reviewer*
- Analista técnico de comandos
    - Responsável por buscar possíveis erros na parte frontal da Saphire e de forma técnica e pesada

#### Pepy#1000 - *Official Designer, Emojis Productor & Events Creator*
- Designer Oficial
    - Responsável por todo e qualquer desenho original da Saphire, produção e alterações de emojis e criação de eventos exclusivos

# 📢 Comandos de Reportes e Sugestões
- No abuso destes comando, a Saphire's Team te colocará na lista de membros bloqueados e você perderá o acesso a estes comandos permanentemente.

`-gif <tema> <link>` - Envie gifs pra serem adicionados ao package da Saphire
- O Link **deve** ser enviado junto ao **tema** no qual você deseja que o Gif seja implementado.
    - Comando exemplo: `-gif Mirai https://imgur.com/F1nJKHZ`

`-sugest` Mande suas ideias para o [servidor central](https://discord.gg/3g8Sa4dUc9)

`-bug [Descrição do ocorrido]` Envie bugs/erros que você notou direto pro Rody#1000, o criador da Saphire

`-donate` Tem um real sobrando? Me ajuda poxa

`-sendcantada` Cantadas pro comando `-cantada`


# 🌐 Saphire Global System

Com o Sistema Global da Saphire, torna possível o compartilhamento de dados entre servidores. Sendo assim, você vai ter as mesmas coisas em todos os servidores em que a Saphire estiver presente.
- Isso se aplica aos Sistemas de [Economia](https://github.com/rodycouto/Saphire-Commands/blob/main/README.md#-o-lindo-sistema-de-economia-global), [Experiência](https://github.com/rodycouto/Saphire-Commands/blob/main/README.md#%EF%B8%8F-level-system---uhuul-level-up) e [AFK](https://github.com/rodycouto/Saphire-Commands/blob/main/README.md#o-bel%C3%ADssimo-afk-global-command)
- Sistemas Locais não existem na Saphire. Então, se você estiver procurando como ativar o *Ranking Local* por exemplo, você não irá achar.
- Não peça por esse sistema. A Saphire's Team **não** vai implentar o sistema local por ser inviável e sem utilidade (até o momento) na Saphire

## ℹ️ Acesso rápido as categorias

Devido a quantidade de comandos, as categorias foram criadas para facilitar o acesso e busca a eles.

Caso você esteja procurando algum comando em especifico, primeiro veja a qual categoria ele pertence. Os nomes das categorias são bem intuitivos então isso não será um problema.

# 🌟 Listinha dos meus lindos comandos

## O Belíssimo AFK Global Command
- Com esse sistema, você pode ficar tranquilo! Quando alguém te chamar, eu vou avisar que você está offline e mostrarei a mensagem que você deixar, tipo um recado.

`-help afk` Informações úteis, uma boa explicação e exemplos fáceis do comando.

`-afk` Ative o AFK no seu servidor sem nenhuma mensagem.

`-afk < sua mensagem >` Ative o AFK com uma mensagem personalizada.

- Ative o AFK em todos os servidores. SIIM, em todos!

`-afk [global/all] < sua mensagem >`
- ##### O AFK System será desativado no momento em que você mandar alguma mensagem.

# 💸 O Lindo Sistema de Economia Global
• Ganhar e perder dinheiro nunca foi tão divertido, agora você pode fazer isso brincando.

Moeda: ⭐RPoints *(Saphire Points)*

- ##### Administração
- `-setpescachannel #canal` Canal para pesca
- `-setminechannel #canal` Canal para mineração
- `-setbuscachannel #canal` Canal para busca
- ~`-seteconomychannel #canal`~ Canal de economia *(Habilita sistema de economia)*

Veja o seu saldo *(adicione @user para ver o saldo de um amigo ***-saldo @user***)*
- Traduções: `-bal` `-balance` `-atm` `-money` `-conta` `-rp` `-saldo`

### ❓ O que tem no Sistema de Economia da Saphire?

- A Saphire não se responsabiliza por dinheiro perdido ou mal usado.

- Lojinha chique: `-shop`
- Inventário Tecnológico: `-slot`
- Pagamentos: `-pay [@user] [valor]`
- Sistema de vendas: `-sell <@user> [item] [quantidade]`
- Deposite: `-dep [valor]/[all]`
- Saque: `-sacar [valor]/[all]`
- Roube: `-rob [@user]`
- Jogo: `-blackjack [valor]`
- Jogue dinheiro no chat: `-lance [valor]`
- Cobre: `-cobrar [@user] [valor]`
- Mendigue: `-esmola`
- Suba no ranking: `-rank money`
- Cace tesouros: `-pesca` `-minerar` `-buscar`
- Libere funções: `-settitulo` `-assaltar [@user]` `-pesca` `-minerar` `-buscar`
- Ajude a Princesa Kaya: `-floresta` *Mais comandos neste comando*
- Duele: `-duelar [@user] [Valor]`
- Jogue na loteria: `-loteria`
- Aprimore seu perfil: `-perfil`
- Envie cartas: `-carta [@user] [Sua mensagem]` ~~*(+5 Reputação)*
- Customize sua cor: `-buy [cor]`

### ❓ Como ganhar dinheiro?

- Ganhe dinheiro: `-work` `-daily` `-vender`
- Aposte na sorte: `-slut` `-cu` `-blackjack` `-crime` `-roleta` `-lotery/-loteria` `-duelar [@user] [valor]`
- Venda itens que você coleta com a `-pesca` `-cavar` `-buscar`, utilize `-sell [item] [quantidade]`. Os baús escondidos são uma boa fonte de renda.
- Ajudar a Princesa Kaya e vender Diamantes são atos preciosos. `-floresta`

- Você pode ser preso fazendo coisas ruins, como roubar ou entrar no mundo do crime, então tome cuidado. Mas se você for preso, você pode tentar `-fugir`, se der azar, a pena aumenta e você não pode mais usar o sistema de economia até o tempo zerar.

### Ajudinha rápida sobre a economia da Saphire

`-help economia` Todos os comandos que aqui estão mais informações que também aqui estão.

`-bjhelp` Não sabe jogar blackjack? Veja [como jogar](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/comojogarbj.md)

`-dogname [NomeDoDoguinho]` Mude o nome do seu cachorrinho após a ajudar a Princesa Kaya `-floresta`

- Traduções suportadas: `-depositar` `-withdraw` `-lotery` `-store` `-buy` `-pagar` `-doar` `-roubar` `-trabalhar` `-prostituir` `-slot` `-mendigar` `-cavar` `-buy` `-comprar` `-inve` `-inventario` `-roubar` `-rol` `-sell` `-loja` `-vender`

# 🌎 Chat Global

Fale com a galera dos outros servidores através do Chat Global.

- `-chat [A sua mensagem global]` Envie sua mensagem para todos os servidores com o sistema ativado.
- `-channel create text/voice canal` Crie um canal se você quiser.
- `-setglobalchat #canal` Escolhe um canal para ser o Global Chat
- `-regraschat` As regras do Global Chat
- `-globalchat` Ajudinha de como ativar o sistema.

##### A Saphire não se responsabiliza por quaisquer mensagem inapropriada! *(Existem moderadores para mutar/banir pessoas do chat global.)*

# ⭐ VIP / Donate - Só os melhores tem

*Antes de tudo, fique ciente de que o VIP System não dá previlégios ou vantagens a ninguém. O VIP System é uma forma de agradecimento e libera funções que não dão vantagens, apenas é legal tê-las.*

Use `-vip` que te mando todas as informações

- ❓ Como obter o VIP?

Simples! Você pode fazer uma doação de [R$1,00](https://mpago.la/2jYiNDg) no Mercado Pago ou fazer um PIX para o meu PicPay, basta digitar `-pix`.

Você pode ter mais informações de como doar enviando `-help` ou `-donate`.

- ❓ O que eu ganho com o VIP?

##### Comandos Vips

- `-shop vip` Um loja exclusiva para os vips
- `-setcolor [cor]` Escolha a cor das suas embeds/mensagens
- `-slot vip` Um slot só pros vips
- `-buy estrela5` Compre a 5º Estrela
- `-esmola` Mendigue dinheiro para os outros
- ... Mais comandos estão sendo escritos ...

- ❓ Como comprovar o pagamento?

Simples! Entre no [meu servidor](https://discord.gg/3g8Sa4dUc9) e fale com o **Rody#1000**, enviando um **print do comprovante** e pronto, você tem seu VIP.

Se quiser mais bônus no servidor principal, fale com **MakolPedro#4369**.

# ⬆️ Level System - Uhuul, level up!
• É isso meeesmo! Level, nível, xp, ranking, tabelinha de xp, tudo isso aí. Lembrando, é interservidor, se você passar de nível no seu servidor, você também vai passar de nível no servidor do seu amigo. 

`-level <@user>` Veja seu level ou o de alguém

`-daily` Pegue sua dose diaria de dinheiro e xp

`-rank xp` Ranking Interservidor de XP Saphire

~`-levelrole [level]`~ Dá um cargo para quem alcançar um level especifico

- Por ser global, os administradores não pode dar ou tirar xp, i'm sorry.

### Family System
- Adicione pessoas ao seu perfil

`-marry [@user]` Se case com alguém

`-divorce [@user]` Se divorcie

`-family` `-nofamily` Informações e comandos que aqui estão

`-family1` `-family2` `-family3 [@user]` Adicione até 3 pessoas a sua familia

`-nofamily1` `-nofamily2` `-nofamily3 [@user]` Tire alguém da sua familia

### Perfil

`-rep [@user]` Dê reputação

`-setstatus [Seu Status]` Defina seu status no seu perfil Exemplo:
*-setstatus Eu sou a lua que brilha no céu*

`-profile` `-perfil < @user >` Veja seu perfil ou o de alguém

`-settitulo [Seu título]` Defina seu título *compre na `-loja`*

`-setsigno` Defina seu signo

`-setniver [DD / MM / AAAA]` Defina seu aniversário

# 💿 Music System

• O Music System foi desativado por que a API do *YouTube/Spotify/SoundCloud* não colaboram comigo.

# 🛠️ Comandos Administrativos
• Isso aqui é um paraiso para os Mods e Adms

#### 🔸 Areazinha da Personalização

`-setxpchannel [#canal]` Envio de mensagens Level Up. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setxpchannel.md)

`-setreportchannel [#canal]` Receba report dos membros. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setreportchannel.md)

`-setleave [#canal]` Canal de saída. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setleave.md)

`-setleavemsg [Sua mensagem de saída]` Escolha uma mensagem legal pra saida. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setleave.md)

`-setwelcome [#canal]` Canal de boas vindas. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setwelcome.md)

`-setwelcome off` `-setleave off` Desative o sistema de boas vindas/saídas

`-setwelcomemsg [Sua mensagem de boas vindas]` Escolha uma mensagem legal de boas vindas. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setwelcome.md)

`-setlogchannel [#canal]` Canal de Relatórios. [Veja mais](https://github.com/rodycouto/RaphyCommands/blob/main/Exemplos/setlogchannel.md) *(Libera funções como ban/mute etc...)*

`-setpescachannel #canal` Canal de pesca

`-setminechannel #canal` Canal de mineração

`-setbuscachannel #canal` Canal de busca

`-setautorole [@cargo]` Selecione um cargo para novos membros do servidor recebe-lo automaticamente

`-setautorole off` Desative o autorole no seu servidor

`-autorole` Veja o estado de autorole do seu servidor

`-role create/delete [Nome do Cargo]` Crie e delete roles

`-role edit name/color [@role] [Novo name/color]` Edite nomes e cor dos cargos

`-role membros [@role]` Veja quantos membros tem em algúm cargo

`-role id [@role]` ID do cargos

#### 🔸 Moderação

`-move [@user]` Move um membro pra sua call

`-addrole [@user] [@cargo]` Dê cargos

`-removerole [@user] [@cargo]` Tire Cargos

`-setnick [@user] [Novo Nome]` Mude o nickname de alguém

`-kick [@user] <razão>` Chute alguém do servidor

`-ban [@user] <razão>` Banir membros

`-unban [ID]` Desban algúm membro *(requer o ID do usuário)*

`-muteinfo` Informações do Mute System

`-mute` Ativa as configurações do comando mute *(Mova a role Muted para um cargo alto)*

`-mute [@user] [10s/m/h] <razão>` Mute alguém

`-unmute [@user] <razão>` Desmuta alguém

`-warn [@user]` Dê Warns *razão em breve*

`-warns [@user]` Confira Warns

`-rwarns` `-resetwarns [@user]` Reset warns de alguém

`-kickvoice [@user]` Tire alguém da call

### 🔸 Chat Commands

`-clonechannel` Cria um canal com o mesmo nome

`-channel create text/voice [#NomeDoCanal]` Crie canais de texto/voz

`-channel topic [NovoTópico]` Edite o tópico do canal

`-channel name [#channel] [novo nome]` Mude o nomes do canais

`-lockdown` Trave o servidor em caso de emergência

`-lock` Trave um canal especifico

`-unlock` Destrave um canal especifico

`-lockcommands <#canal>` Bloqueie meus comandos em canais especificos

`-unlockcommands <#canal>` Libere meus comandos em canais que foram bloqueados

`-clear [all]` Limpe todo o chat

`-clear [1~99]` Apague de 1~99 mensagens

`-clear <@user> [1~99]` Apague 1~99 mensagens de alguém especifico

`-clear <bot>` Apague mensagens de bots

`-clear <images> [1~99]` Apague somente imagens/videos/gifs

- ##### Mensagens acima de 14 dias não serão apagadas por limitações do Discord.

`-slowmode [Tempo em segundos]` Ative o modo lento

### 🔸 Saphire Comandos Exclusivos

`-setprefix [NovoPrefixo]` Muda meu prefixo

`-prefix` Informações sobre o prefixo

`-resetprefix` Reseta meu prefixo para ( - )

`-dm` `-pv [@user] [Mensagem]` Comando Travado

`-ideiasaphire [Sua ideia]` Envia sua ideia diretamento pro servidor central. 100% de certeza de que você será ouvido.

`-gif [linkdogif.com]` Envie gif e aumente os packs da Saphire

### 🔸 Interação com os server

`-criarcomando [NomeDoComando] [O que eu vou responder]` Cria um comando só pro servidor

`-deletecomando [NomeDoComando]` Deleta um comando criado

~`-listadecomandos`~ Em breve

`-send [#canal] [Sua mensagem]` Envie mensagens para outros canais

`-say [Sua mensagem]` Mande mensagens atráves de mim

`-sorteio [20s/m/h/d] [#CanalDoSorteio] [Prêmio]` Faça sorteios

# 🎮 Link de Games

• Acesse os principais jogos rapidamente

`-amongus`

`-brawlhalla`

`-brawlstars`

`-clashroyale` `-clash`

`-counterstrike` `-cs`

`-ddtank`

`-freefire` `-ff`

`-gartic`

`-genshinimpact` `-genshin`

`-gta`

`-habbo`

`-leagueoflegends` `-lol`

`-summoners` `-sumwar`

`-mario`

`-mobilelegends` `-mbl`

`-minecraft`

`-paladins`

`-rocketleague`

`-transformice` `-tfm`

`-uno`

`-valorant`

# 📺 Animes

- Esses comandos estão sendo desenvolvidos ainda, não se preocupe

~`-anime [Seu anime]`~ Pesquisa de animes

`-ind` Indicação de Animes (+2000 Animes)

`-sao` Fotos/gifs de Sword Art Online

`-senpai` Meu lindo e perfeito senpai

`-quiznaruto` Quiz de Naruto pra quem é viciado

`-quizonepiece/op` Tem de One Piece também

`-quizkimetsu` Kimetsu no Yaiba também está na lista.

#### Pack de Gifs/Fotos

`-asuna` | `-chika` | `-nezuko` | `-zerotwo/-zt`

`-gojo` | `-zenitsu` | `-naruto` | `-itachi`

`-kimetsu` Pack de Gifs de Kimetsu No Yaiba

- Em breve: `-kirito` e mais 36 packs *(Tô com preguiça de escrever)*
# 💡 Comandos Gerais

• Comandos úteis e que podem ajudar muito

`-covid < br >` Informações mundiais sobre a COVID-19

### 🔸 Servidor

`-invite` `-inv` Me coloque no seu servidor

`-ideia [Sua ideia]` Mande ideias pro seu Servidor para ser votada

`-report <@user> [O motivo do seu report]` coisas pra Staff do Servidor

`-serverinfo` Informações do Servidor

`-dono` `-owner` Veja quem é o dono do servidor

`-invites` Veja quantas pessoas cada membro convidou pro servidor *(Criador do link que ganha o invite)*

### 🔸 Comandinhos da Saphire

`-ping` Pong!

`-uptime` Tempo que eu estou acordada

`-botinfo` Informações sobre mim

`-sup` `-support` `-suporte` Minha central de Suporte

### 🔸 Usuário

`-avatar <@user>` Veja sua foto de perfil ou a de alguém

`-id <@user>` Veja o ID

`-spotify` `-spot <@user>` Veja a música que alguém está escutando

`-user <@user>` Veja o nome de usúario de alguém

`-userinfo <@user>` Veja informações detalhadas de alguém

### 🔸 Fun

`-Jokenpo` `-j < pedra > < papel > < tesoura >` Jooo keeen po!

`-coinflip < cara > < coroa >` Cara ou coroa?
 
~`-ascii [Sua mensagem]`~ Muito doido

`-dado` `-roll [1...4]` Role até 4 dados

`-frase` Filosofia pura

~`-changemymind [Sua frase]`~ Changemymind Meme (Not Working)

`-history` Histórias criadas pelo pessoal do Discord

### 🔸 Util

`-cat` Gatinhos são úteis, tá bom? ;-;

`-clima [SuaCidade]` Veja o clima da sua cidade

`-cal [10 + 10]` Calculadora poxa

`-translate` `-t [pt][en][fr] [A frase que deseja traduzir]` Traduza com o Google Tradutor *Exemplo: -t pt What are you mean with that?*

`-invisible` `-invisivel` Não há nada aqui

`-piada` kkkkkk q u a l i d a d e

`-imc [peso] [altura]` (Peso e Altura sem ponto e virgula) Veja seu imc *Exemplo: -imc 75 181*

### 🔸 Random

`-advice` This command is completelly in english

`-conselho` Esse comando é completamente em português

`-invisivel` `-invisible` Invisivel

`-nota [@user]` Dê nota a alguém

`-boom` Só faz boom

`-cantada` Cantadas muito boas

`-explosion` Megumiiin

`-feet` Pézin

`-servidores` Servidores que usam a Saphire

`-livia` Gay

`-loli` Lolicon

`-stonks` `-notstonks` Stonks esse comando

`-puppy` Fofiiiinho

# ❤️ Comandos de Reações
• Demonstre reações por gifs no servidor

`-clap` Aplaudir é legal

`-cry` Chorar as vezes faz bem

`-dance` Dance Dance

`-ery` Puff.. kkkk

`-facepalm` Decepcionado(a)

`-fury` Mostre sua furia!

`-glare` Olhar Pistola

`-happy` Feliiiz

`-kkk` kkkkkk

`-pout` Fazendo Bico

`-run` Coooorre

`-sad` Triste

`-shy` Vergonha

`-sleep` Que soninhoo

`-smile` Está feliz? Sorria!

`-surprise` Mas o que é isso?

`-thin` Oque eu faço... Pensando...

`-tired` que cansaço!

`-trig` TRIGGEEEEER

- Todos tem traduções para português.

# 🤝 Comandos de Interação
• Interaja com as pessoas atráves de gifs sensacionais

`-votação` `-votar` `-vote [O que deseja ser votado]` Abra um votação

`-baka [@user]` Chame de Baka!

`-morder` `-bite [@user]` Mordidinha

`-dedo [@user]` Educação

`-saudar` `-comprimentrar` `-greet [@user]` Comprimento

`-fight [@user]` Briga, briga, briga!

`-lutar [@user]` Lutinha saudavel

`-abraço` `-abraçar` `-hug [@user]` Abraçar é tão boom

`-matar` `-kill [@user]` Mate!

`-beijo` `-beijar` `-kiss [@user]` Hmmm, beijo é?

`implorar` `-onegai [@user]` Por favoor

`-carinho` `-pat [@user]` Cariiiinho

`-rob [@user]` Roubar é tão feio...

`-tapa` `-slap [@user]` Tapããão

`-slaap [@user]` Tapão em foto

`-love [@user]` Mande seu amor secretamente

`-ery [@user]` Deboche

`-lamber` `-lick [@user]` Lambidinha

`-chupar` `-suck [@user]` Chupada

`-tocaai` `-highfive [@user]` Toca Aqui

`-cutucar` `-poke [@user]` Cutucadinha

`-atirar` `-shoot [@user]` Atire para matar

`-shrug [@user]` Fazer oque ne

`-olhar` `-stare [@user]` Olhar fixo

`-pisar` `-stomp [@user]` Pise em alguem

`-affect [@user]` Isso não afeta o bebê

`-explodir [@user]` Exploda alguém

- Todos comandos de interação tem traduções, caso queira adicionar mais alguma tradução, [clique aqui](https://forms.gle/vtJ5qBqFDd9rL5JU8)

# 📝 Quiz - BETA
• O Saphire Quiz é um jogo simples e divertido de brincar. Tendo 2 modos, é possível brincar bastante. Jogar com os amigos é ainda mais divertido!!!

`-quiz` Você terá apenas 15 segundos para responder

`-quiznaruto` Você conhece Naruto? Tem 15 segundos também

`-quizonepiece/op` Tem 15s pro One Piece também

`-quizkimetsu` Tem 15s pro Kimetsu também

### Em breve
`-quizanimes` `-quizcinema` `-quizhistoria` `-quizch` -> corpo humano

#### Informações Adicionais

Quiz: 27 Perguntas

Quiz Naruto: 17 Perguntas

Quiz One Piece: 15 Perguntas (24 perguntas em breve)

Quiz Kimetsu no Yaiba: 15 perguntas

# 📁 Saphire Assistence
• Pessoas que ajudaram na construção da Saphire

[Servidor Central](https://discord.gg/3g8Sa4dUc9) | [Host Atual](https://discloudbot.com/)

### Moderação do Servidor Saphire
MakolPedro#4369 | ツ𝑫𝒐𝒎𝒂#6556 | ! Kokushibou#2447 | Raffah#1751

### Créditos a todos que ajudaram

Rody#1000 | `Construção do Código e Pesquisa`

Gowther#9233 | `Start Cooper` *~Developer acess & Gif Collector*

Spray#7194 | `-lock | -unlock` *~ideia*

Heryson#6565 | `AllowedMentions` *~conselho & ajuda*

𝑅𝑈 𝑀𝑖𝑛𝑒?#2790 | `Ideias nos Comandos de Administração` *~ideia*

LucasLuquisquiss#4643 | `-frase` *~ideia*

João da Cilada#7041 | `-love` *~ideia*

MakolPedro#4369 | `-help games` `-quizonepiece` `Atualizador do Help` *~ideias, developer, Perguntas/Respostas Quiz*

Ceci#7184 | `-doar itens` *~ideia*

Giacometti#4849 | `-feet` *~ideia*

! Kokushibou#2447 | ツ𝑫𝒐𝒎𝒂#6556 | `-quizkimetsu` *~ideia + Perguntas/Respostas Quiz*

Dog Comunista#2185 | `-lockcommands` *~ideia*

Coiny#9056 | `Painel Interativo` *~ideia & support*

CALORIES#2822 | `Music System Helper` ~ USA *~ideia, support, developer*

### Caça Bugs

Casado✋🏻😎#9113: `5 Bugs`

Heryson#6565: `2 Bugs`

Pitticco#1827: `1 Bug`

ツ𝑫𝒐𝒎𝒂#6556: `29 Bugs`

Bαყʑι ッ#5485: `4 Bugs`

MakolPedro#4369: `49 Bugs`

! Kokushibou#2447 `15 Bugs`

### Gifs / Emojis

[@milkmochabear](https://www.instagram.com/milkmochabear)

Foto de perfil: [Sayu](https://br.pinterest.com/pin/849843392186157763/)

ツ𝑫𝒐𝒎𝒂#6556 `Emojis servidor`

Gowther#9233 `Gifs Saphire`

dspofu#8041 `Emojis servidor`

Hype#0004 `Emojis Saphire`

! Kokushibou#2447 `Emojis servidor`

¸,ø¤{•ลℓเ¢э-µ¢ђเђล}ø¤º°#6146 `Emojis servidor`

### Designers

Yma#5175, Rody#1000, Gowther#9233, Sayu