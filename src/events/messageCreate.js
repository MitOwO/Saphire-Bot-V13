...
//         message.channel.send(`${e.Check} | ${message.author}, percebi que você mudou de nome. Acabei de atualizar seu nome no meu banco de dados.`)
//     }
...
//             if (command.category === 'economy') { if (sdb.get(`Users.${message.author.id}.Timeouts.Preso`) !== null && 1500000 - (Date.now() - sdb.get(`Users.${message.author.id}.Timeouts.Preso`)) > 0) return message.reply(`${e.Cadeia} Você está preso! Liberdade em: \`${time.minutes}m e ${time.seconds}s\``) }
//         }
