...
client.on("guildCreate", async (guild) => {
    ...
    let ChannelInvite = await guild.channels.cache.find(channel => channel.type === 'GUILD_TEXT' && channel.permissionsFor(guild.me).has(Permissions.FLAGS.SEND_MESSAGES))

    let ChannelId = config.guildCreateChannelId

    function WithChannel() {

        ChannelInvite.createInvite({ maxAge: 0 }).then(ChannelInvite => {
    ...
    ChannelInvite ? WithChannel() : WithoutChannel()
})
