...
module.exports = {
    ...
    run: async (client, message, args, prefix, db, MessageEmbed, request) => {
        ...
        // 2592000000 ---> 1 mês
        let time = ms(2592000000 - (Date.now() - db.get(`${message.author.id}.Timeouts.Monthly`)))
        if (db.get(`${message.author.id}.Timeouts.Monthly`) !== null && 2592000000 - (Date.now() - db.get(`${message.author.id}.Timeouts.Monthly`)) > 0)
        ...
        function InsideGuild() {
            ...
            db.set(`${message.author.id}.Timeouts.Monthly`, Date.now())
