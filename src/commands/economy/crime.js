...
module.exports = {
    ...
    run: async (client, message, args, prefix, db, MessageEmbed, request) => {
        ...
        let Helpier = db.get(`${message.author.id}.Slot.Helpier`)
        ...
        if (!db.get(`${message.author.id}.Slot.Balaclava`))
        ...
        return message.reply(`${e.QuestionMark} | Diga o **NÚMERO** do lugar que você deseja assaltar${Mg}\n \n🏠 1 Casa\n🏦 2 Mansão\n🏛️ 3 Prefeitura\n🏣 4 Cartório\n📨 5 Correios\n💍 6 joalheria\n🏢 7 Shopping\n🏭 8 Fábrica\n🏩 9 Motel\n🪙 10 Banco\n${e.PandaBag} 11 Arrastão\n🚆 12 Trem\n${e.Deny} 0 Cancelar`).then(msg => {
            ...
            collector.on('collect', m => {
                ...
                function NewCrimeStart(value) {
                    switch (value) {
                        case 0: msg.edit(`${e.Deny} | Comando cancelado.`).catch(() => { }); db.delete(`${message.author.id}.Timeouts.Crime`); break;
                        case 1: Casa(); break;
                        case 2: Mansao(); break;
            ...
            function Casa() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou uma casa.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }
                Result() <= 50 ? Win(Math.floor(Math.random() * 3000)) : Lose()
            }

            function Arrastao() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} fez um arrastão.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }

                Result() <= 70 ? Win(Math.floor(Math.random() * 2000)) : Lose()
            }

            function Trem() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} assaltou um trem.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }
                Result() <= 40 ? Win(Math.floor(Math.random() * 6000)) : Lose()
            }

            function Mansao() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou uma mansão.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }
                Result() <= 10 ? Win(Math.floor(Math.random() * 150000)) : Lose()
            }

            function Prefeitura() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou a prefeitura.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }

                Result() <= 35 ? Win(Math.floor(Math.random() * 10000)) : Lose()
            }

            function Cartorio() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou o cartório.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }

                Result() <= 41 ? Win(Math.floor(Math.random() * 14000)) : Lose()
            }

            function Correios() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou os correios.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }

                Result() <= 32 ? Win(Math.floor(Math.random() * 8000)) : Lose()
            }

            function joalheria() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou uma joalheria.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }

                Result() <= 15 ? Win(Math.floor(Math.random() * 300000)) : Lose()
            }

            function Shopping() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou um shopping.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }

                Result() <= 34 ? Win(Math.floor(Math.random() * 80000)) : Lose()
            }

            function Fábrica() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou uma fábrica.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }

                Result() <= 40 ? Win(Math.floor(Math.random() * 16000)) : Lose()
            }

            function Motel() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou um motel.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }

                Result() <= 35 ? Win(Math.floor(Math.random() * 30000)) : Lose()
            }

            function Banco() {
                function Win(amount) { db.add(`Balance_${message.author.id}`, amount); return msg.edit(`${e.PandaBag} | ${message.author} roubou um banco.\n+ ${amount} ${Moeda(message)}`).catch(() => { }) }
