...
module.exports = {
    ...
    run: async (client, message, args, prefix, db, MessageEmbed, request, sdb) => {
        ...
        if (['continue', 'continua'].includes(args[0]?.toLowerCase())) {
            if (!Cachorro) { return message.reply(`${e.Deny} | Você ainda não achou o Brown! \`${prefix}floresta cammum\``) }
            if (Cachorro && !Remedio) return message.reply({ embeds: [Cammum3] })
            if (Cachorro && Remedio && !Bola) return message.reply({ content: `${e.Deny} | Você ainda não achou a bolinha do Brown.`, embeds: [CammumComRemediosDoVelhoWelter] })
            if (Cachorro && Bola && Remedio) return message.reply({ content: `${e.Check} | Você já completou está missão! Use \`${prefix}floresta final\``, embeds: [CammumComRemediosDoVelhoWelter] })
        }
