...
module.exports = {
    ...
    run: async (client, message, args, prefix, db, MessageEmbed, request) => {
        ...
        let timeout = 1200000
        let daily = db.get(`${message.author.id}.Timeouts.Roubo`)
        if (daily !== null && timeout - (Date.now() - daily) > 0) {
            ...
            if (!isNaN(args[0]) && args[0]) {
                let u = client.users.cache.get(args[0])
            ...
            function Rob(u) {
                ...
                function win(u) {
                    db.subtract(`Balance_${u.id}`, amount1); db.add(`Balance_${message.author.id}`, amount1); db.set(`${message.author.id}.Timeouts.Roubo`, Date.now())
