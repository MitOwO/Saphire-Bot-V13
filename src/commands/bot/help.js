...
module.exports = {
    name: 'help',
    aliases: ['comandos', 'commands', 'h', 'ajuda', 'socorro'],
    ...
    run: async (client, message, args, prefix, db, MessageEmbed, request) => {
        ...
        message.reply({ content: `${message.author}`, embeds: [PrincipalEmbed], components: [painel] }).then(msg => {
            ...
            coletor.on('end', async (collected) => {
                const Embed = new Discord.MessageEmbed().setColor('RED').setTitle(`${e.Deny} Central de Ajuda ${client.user.username}`).setDescription(`Sessão encerrada por Tempo Expirado.\nCaso queira ver o help novamente, basta usar \`${prefix}help\``)
