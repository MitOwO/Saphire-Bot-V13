        ...
        return message.reply({ embeds: [FaqEmbed], components: [FaqPainel] }).then(msg => {
            ...
            function Titulos() {
                const TitulosEmbed = new MessageEmbed()
                    .setColor('#246FE0')
                    .setTitle('Títulos')
                    .setDescription(`${e.SaphireObs} Os títulos são bem díficeis de conseguir. Atualmente, os membros que possuem títulos não passam de 10. Os títulos pode ser obtidos estando em primeiro lugar do ranking, participando de eventos no [servidor principal](${config.ServerLink}) ou sendo da parde da ${client.user.username}'s Team.`)
