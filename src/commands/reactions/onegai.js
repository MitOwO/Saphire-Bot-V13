...
module.exports = {
  ...
  run: async (client, message, args, prefix, db, MessageEmbed, request) => {
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Onegai[Math.floor(Math.random() * g.Onegai.length)]); msg.edit({ embeds: [embed] }).catch(err => { return }) })
