...
module.exports = {
  name: 'die',
  aliases: ['suicide', 'suicidio', 'morrer', 'morri'],
  category: 'reactions',
  UserPermissions: '',
  ClientPermissions: ['EMBED_LINKS', 'MANAGE_MESSAGES'],
  emoji: '😵',
  usage: '<morri> [motivo]',
  description: 'Morri, adeus',

  run: async (client, message, args, prefix, db, MessageEmbed, request) => {

    if (request) return message.reply(`${e.Deny} | ${f.Request}`)
    let rand = g.Morrer[Math.floor(Math.random() * g.Morrer.length)]
    let texto = args.join(" ")
    if (!texto) texto = `${message.author} morreu`
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Morrer[Math.floor(Math.random() * g.Morrer.length)]); msg.edit({ embeds: [embed] }).catch(err => { return }) })
