...
module.exports = {
  ...
  run: async (client, message, args, prefix, db, MessageEmbed, request) => {
    ...
    return message.reply({ embeds: [embed] }).then(msg => {
      ...
      Collector.on('collect', (reaction, user) => { embed.setImage(g.Cry[Math.floor(Math.random() * g.Cry.length)]); msg.edit({ embeds: [embed] }).catch(err => { return }) })
