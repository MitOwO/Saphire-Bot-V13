    ...
    run: async (client, message, args, prefix, db, MessageEmbed, request) => {
        ...
        message.reply({ embeds: [embed] }).catch(() => { }).then(embedmsg => {
            ...
            function SetSignoProfile(signo) {
                if (db.get(`${message.author.id}.Perfil.Signo`) === signo) return message.channel.send(`${e.Info} | ${message.author}, este já é o seu signo definido.`)
                db.set(`${message.author.id}.Perfil.Signo`, `${signo}`)
