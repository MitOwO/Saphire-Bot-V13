...
module.exports = {
  ...
  run: async (client, message, args, prefix, db, MessageEmbed, request) => {
    ...
    } else if (['all', 'tudo'].includes(args[0])) {

      return message.reply(`${e.Loading} | Sessão em reforma.`)

      // if (!message.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR)) {
      //   return message.reply(`${e.Deny} | Este comando apaga completamente o chat, logo, este comando é fechado apenas para Administradores.`)
      // }
      // if (args[1]) { return message.reply(`${e.Deny} | Nada além do argumento "${args[0]}"`) }

      // let messages = 0
      // let i = true
      // while (i) {
      //   if (!message.channel) i = false
      //   let deleteAble = message.channel.messages.fetch({ limit: 100 }).catch(err => { i = false; message.channel.send(`${e.Attention} | Houve algum tipo de "erro" na execução:\n\`${err}\``) })
      //   if (deleteAble.size < 99) {
      //     await message.channel.bulkDelete(deleteAble).catch(err => { i = false; message.channel.send(`${e.Attention} | Houve algum tipo de "erro" na execução:\n\`${err}\``) })
      //     messages += deleteAble.size
      //     i = false
      //     message.channel.send(`${e.Check} | Deletei um total de ${messages} mensagens.\n${e.Info} | Mensagens acima de 14 dias não podem ser apagadas por bots. (Limitações do Discord)`)
      //     messages = 0
      //     return
      //   }
      //   message.channel.bulkDelete(deleteAble).catch(err => { i = false; message.channel.send(`${e.Attention} | Houve algum tipo de "erro" na execução:\n\`${err}\``) })
      //   messages += deleteAble.size
      // }
